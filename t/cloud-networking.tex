...
Another way how to accomplish unlimited \Ac{VM} migrations is to use routing instead of switching for virtual machine connectivity. This method uses temporary and fixed \Ac{IP} addresses. Hypervisor do not have to be in same L3 network and can use different address range too. Temporary address is assigned according to \Ac{VM}'s location and it changes during migration.  Fixed address is routed to \Ac{VM} and this address is not changed during migration, because changes are made only in routing tables. There is used any routing protocol, e.g. \Ac{OSPF}, to provide correct routing of fixed to current temporary address. Higher layer is used to get more flexibility than lower level can offer. Main drawback of this solution is additinal complexity caused by routing and address swap after migration is longer than it should be, because it takes some time to propagate routing to new temporary address. It is not easy to perform live migration, because it is neccesary to change \Ac{IP} address of \Ac{VM}'s interfacace and open sessions are terminated.
